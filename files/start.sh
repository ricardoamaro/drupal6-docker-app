...
if [ ! -f /var/www/html/sites/default/settings.php ]; then
  ...
  echo "root:${ROOT_PASSWORD}" | chpasswd
	...
	cd /var/www/html
	${DRUSH} site-install default -y --account-name=admin --account-pass=admin \
  --db-url="mysqli://drupal:${DRUPAL_PASSWORD}@localhost:3306/drupal" \
  --site-name="Drupal6 docker App" | grep -v 'continue?'
...
echo "Please report any issues to https://github.com/ricardoamaro/drupal6-docker-app"
